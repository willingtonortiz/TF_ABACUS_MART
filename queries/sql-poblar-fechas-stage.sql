...
BEGIN TRANSACTION
	...
	--Esta l�nea permite agregar 2 a�os mas al a�o actual, se deja como ejemplo
    ...
    WHILE (@FechaEvaluada <= @FechaHasta) --Mientras @FechaEvaluada sea menor o igual a la @FechaHasta
	   BEGIN
            --Obtiene el valor para el campo SK_FECHA en formato n�mero YYYYMMDD de la @FechaEvaluada
			SELECT @Sk_Fecha = YEAR(@FechaEvaluada)*10000 + MONTH(@FechaEvaluada)*100 + DATEPART(dd, @FechaEvaluada)
            --Obtiene el a�o de la @FechaEvaluada
			SELECT @Anio = DATEPART(yy, @FechaEvaluada)
			--Obtiene la descripci�n del semestre de la @FechaEvaluada
			SELECT @Semestre = CASE
							     WHEN Month(@FechaEvaluada) <= 6 THEN 'Semestre 1' 
								 ELSE 'Semestre 2'
							   END
             --Obtiene la descripci�n del trimestre de la @FechaEvaluada
			SELECT @Trimestre = CASE
								 ...
								 ELSE 'Trimestre 4'
							   END
			 --Obtiene la descripci�n del mes de la @FechaEvaluada
			SELECT @Mes = CASE
			...
			--Obtiene la posici�n del mes en el a�o de la @FechaEvaluada
			SELECT @NumMesAnio = Month(@FechaEvaluada)

			--Obtiene la posici�n del d�a de la semana de la @FechaEvaluada
			SELECT @NumDiaSemana = DATEPART(dw, @FechaEvaluada)

			--Obtiene la posici�n del d�a del mes de la @FechaEvaluada
			SELECT @NumDiaMes = DAY(@FechaEvaluada)

			--Inserta el registro con los valores extra�dos de la @FechaEvaluada en la tabla Dim_Fechas
			...
			--Incremento el valor de la @FechaEvaluada en un d�a
